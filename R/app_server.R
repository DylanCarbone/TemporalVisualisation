...
server <- function(input, output) {
  ...
  species_with_images <- c("badger", "fox", "muntjac", "hedgehog", "dog", "human", "deer")
  ...
  data_input <- reactive({
    ...
    all_data %>%
      ...
      mutate(icon_path = paste0("images/", ifelse(input$species %in% species_with_images == T, input$species, "other"), ".png")) %>%

      # Add new column with species names which correspond to species information in list
      mutate(species_info = paste0(ifelse(input$species %in% species_with_images == T, input$species, "other")))
  })
  ...
  output$mymap <- renderLeaflet(
    leaflet() %>%
      ...
      # Add icons to map
      # Construct icons
      addMarkers(icon  = makeIcon(data_input()$species_info,
                 ...
                 popup = paste0("Here there was ", data_input()$total_count, " ", data_input()$species, " sightings")))
